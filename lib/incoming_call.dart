...
import 'package:vitatraq_chart/active_call_video.dart';
  ...
  void _acceptCall(BuildContext context) {
    CometChat.acceptCall(call.sessionId.toString(), onSuccess: (acceptedCall) {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => ActiveVideoCallScreen(call: acceptedCall),
        ),
      );
  ...
  void _rejectCall(BuildContext context) {
    CometChat.rejectCall(call.sessionId.toString(), CometChatCallStatus.rejected, onSuccess: (rejectedCall) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Call rejected successfully.')),
      );
    }, onError: (CometChatException error) {
